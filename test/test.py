...
@cocotb.test()
async def test_independent_firing(dut):
    """Test neurons firing independently"""
    await initialize_dut(dut)
    
    # Set base current (ui_in[7:3]) and no coupling pattern (ui_in[2:0])
    dut.ui_in.value = 0b10000000  # High base current, pattern 0
    dut.uio_in.value = 0  # No coupling strength
    
    # Monitor for 100 clock cycles
    spikes_n1 = []
    spikes_n2 = []
    
    for _ in range(100):
        await RisingEdge(dut.clk)
        spikes_n1.append(dut.uo_out.value.integer & 0x1)
        spikes_n2.append((dut.uo_out.value.integer & 0x2) >> 1)
    
    # Check if both neurons fired at least once
    assert sum(spikes_n1) > 0, "Neuron 1 never fired"
    assert sum(spikes_n2) > 0, "Neuron 2 never fired"

@cocotb.test()
async def test_coupled_firing(dut):
    """Test neurons with coupling"""
    await initialize_dut(dut)
    
    # Set base current and excitatory coupling pattern
    dut.ui_in.value = 0b10000001  # High base current, pattern 1 (excitatory)
    dut.uio_in.value = 0xFF  # Maximum coupling strength
    ...
    for _ in range(100):
        ...
        # Print spikes for visualization
        if dut.uo_out.value.integer > 0:
            print(f"Time {_}: Spikes = {bin(dut.uo_out.value.integer)}")
...
@cocotb.test()
async def test_membrane_potential(dut):
    """Test membrane potential behavior"""
    await initialize_dut(dut)
    
    # Set moderate base current
    dut.ui_in.value = 0b01000000
    
    # Monitor membrane potential for 20 cycles
    for i in range(20):
        await RisingEdge(dut.clk)
        potential = dut.uio_out.value.integer
        print(f"Time {i}: Membrane Potential = {potential}")